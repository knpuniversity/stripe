...
`billingPeriodEndsAt` column. *All* we need to do is set this when the subscription
is first created. Ok, we also need to update it each month when the subscription is
renewed - but we'll talk about that later with webhooks.

The `Subscription` is created - or retrieved - right here in `SubscriptionHelper`.
This is the spot to set that date.

And hey! This method is passed a `\Stripe\Subscription` object. Let's check out the
API docs to see what that gives us. Oh yes, it has a `current_period_end` property,
which is a UNIX timestamp. Bingo!
...
`$stripeSubscription->current_period_end`. Now, pass *that* into the
`activateSubscription()` method as a new argument.

Open that function in `Subscription` and add a `\DateTime` argument called `$periodEnd`.
Set the property with `$this->billingPeriodEndsAt = $periodEnd`. Done!
